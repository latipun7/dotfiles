# Generated by Powerlevel10k configuration wizard on 2026-05-25 at 22:59 WIB.
...
function prompt_bun_version() {
  # Only show if inside a project directory
  if [[ ! -f package.json && ! -f bun.lockb && ! -f bun.lock ]]; then
    return
  fi

  # Only show if Bun is installed
  if (( ! $+commands[bun] )); then
    return
  fi

  local bun_ver=$(bun --version 2>/dev/null)

  # Using \ue76f for the official Bun Nerd Font icon
  p10k segment \
    -f 007 \
    -i $'\ue76f' \
    -t "${bun_ver}"

...
M.map = function(mode, target, source, opts) vim.keymap.set(mode, target, source, get_map_options(opts)) end
...
M.t = function(str) return vim.api.nvim_replace_termcodes(str, true, true, true) end

M.input = function(keys, mode) api.nvim_feedkeys(M.t(keys), mode or "m", true) end
...
local change_template_string_quotes = function()
  ...
  M.gfind(api.nvim_get_current_line(), "[\"']", function(pos)
    ...
    elseif pos < col then
      ...
      if (pos - col) > (quote_start - col) then quote_start = pos end
...
local css_to_js = function(opts)
  ...
  for i, line in ipairs(api.nvim_buf_get_lines(0, start_line, end_line, false)) do
    ...
    if line:sub(#line) == "," then goto continue end
    -- ignore comments
    if line:find("%/%*") then goto continue end

    local indentation, name, val = line:match("(%s+)(.+):%s(.+)")
    -- skip non-matching lines
    if not (name and val) then goto continue end
  ...
  if not did_convert then M.warn("css-to-js: nothing to convert") end
end

...
wezterm.on("update-right-status", function(window)
  local eff_con = window:effective_config()
  local session_name = "L"
  local session_color = mocha.ansi[1]
  for _, v in ipairs(eff_con.unix_domains) do
...
if wezterm.target_triple == "x86_64-pc-windows-msvc" then
  ...
  config.font = wezterm.font_with_fallback({ "CaskaydiaCove NF", "Monospace" })
  config.background[2].source.File = "C:\\Users\\M S I\\Pictures\\halal-indonesia.png"
elseif wezterm.target_triple == "x86_64-unknown-linux-gnu" then
  config.enable_wayland = false
  config.background[2].source.File = "/home/data/pictures/random/dyah-latif.png"
else
  config.font = wezterm.font_with_fallback({ "monospace", "Symbols Nerd Font" })
end

...
function M:peek()
  ...
  if not child then return self:fallback_to_builtin() end
  ...
  repeat
    ...
    i = i + 1
    if i > self.skip then lines = lines .. next end
...
function M:fallback_to_builtin()
  ...
  if bound then ya.manager_emit("peek", { bound, only_if = self.file.url, upper_bound = true }) end
end
